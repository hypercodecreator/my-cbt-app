// =========================================================
// [v46.0.0] app-parser.js: Multi-Set Block Extractor
// (Flawlessly parses unlimited consecutive question sets)
// =========================================================
...
window.showBulkAddModal = function() { 
    ...
    <div class="modal" style="max-width:1000px; width:95%; background:#fff; border-radius:20px; padding:35px; box-shadow:0 10px 40px rgba(0,0,0,0.15); position:relative; z-index:100000;">
        <div style="text-align:center; margin-bottom:20px;">
            <h2 style="color:#4f46e5; margin-bottom:10px; font-size:1.8em;">🤖 퀀텀 스마트 주입기 (다중 세트 무한 주입)</h2>
            <p style="color:#64748b; margin-bottom:10px;">여러 개의 문제 세트(1~7번 덩어리)를 한 번에 넣어도 <b>전부 분해하여 저장</b>합니다.</p>
        </div>
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px; margin-top:25px;">
            <button class="button primary-button" style="height:60px; font-size:1.2em; border-radius:12px; background:#4f46e5; border:none;" onclick="window.processUnifiedBulkAdd()">🚀 무한 세트 주입 시작</button>
...
window.processUnifiedBulkAdd = async function() {
    ...
    try {
        ...
        // 🚨 1. 전체 텍스트를 "1. 핵심 리마인드"를 기준으로 거대한 세트(Set) 단위로 분해합니다!
        // 이렇게 하면 3문제를 넣든 100문제를 넣든 다 쪼개집니다.
        ...
        for (let setIdx = 0; setIdx < problemSets.length; setIdx++) {
            ...
            // 🚨 [5] 사례 분석: 극한의 제1원칙! 제목(단어) 따위 신경 안 쓰고 무조건 1,2,3,4번 줄에 꽂음.
            if (s5) {
                let sObj = { title:'사례 분석', category:'미분류', sit_c:'', sit_n:'', pri_c:'', pri_n:'', sta_c:'', sta_n:'', pnt_c:'', pnt_n:'' };
                let s5Clean = s5.replace(/[-=|]{3,}/g, '');
                
                // 떡진 텍스트 심폐소생술 (안전을 위해 주요 키워드 앞에만 엔터 주입)
                if (!s5Clean.includes('|') && !s5Clean.includes('\t')) {
                    s5Clean = s5Clean.replace(/(상태|상황|원인|작용\s*원리|에너지\s*설정|이유|해당\s*혈관|혈색\s*양상|처치|판단|구조\s*성격|생리적\s*상태|학습\s*포인트|판단\s*근거)/g, '\n$1 | ');
                } else {
                    s5Clean = s5Clean.replace(/([^\n])\s*(상태|상황|원인|작용\s*원리|에너지\s*설정|이유|해당\s*혈관|혈색\s*양상|처치|판단|구조\s*성격|생리적\s*상태|학습\s*포인트|판단\s*근거)\s*(?:\||\t)/g, '$1\n$2 | ');
                }
                ...
                if (dataRows.length > 0) {
                    ...
                    // 단어 매핑 버림. 무조건 위에서부터 1번째, 2번째, 3번째, 4번째 줄이다!
            ...
            if (s6) {
                ...
                if (!s6Clean.includes('|') && !s6Clean.includes('\t')) {
                    s6Clean = s6Clean.replace(/(QRS 모양|주요 원인|시각적 특징|리듬 형태|QRS 폭|규칙성|이상파형|단상파형|비교 항목|구조 명칭|태아기 기능|출생 후 변화|성인 존재 여부|특수 구조|연결 지점|우회 대상 장기|특징)/g, '\n$1 |');
                } else {
                    s6Clean = s6Clean.replace(/(J|s|초|점|\)|에너지|여부|규칙성|적|음|조동|세동|빈맥|모양|원인|특징|변함|형태|폐쇄됨|판막\)|통로)\s*([가-힣A-Z])/g, (m,p1,p2) => {
