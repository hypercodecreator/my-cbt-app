// =========================================================
// [v45.0.0] app-parser.js: Multi-Block Bulk Injection Engine
// (Splits multiple quizzes, reconstructions, and case studies)
// =========================================================
...
window.showBulkAddModal = function() { 
    ...
    <div class="modal" style="max-width:1000px; width:95%; background:#fff; border-radius:20px; padding:35px; box-shadow:0 10px 40px rgba(0,0,0,0.15); position:relative; z-index:100000;">
        <div style="text-align:center; margin-bottom:20px;">
            <h2 style="color:#4f46e5; margin-bottom:10px; font-size:1.8em;">🤖 퀀텀 스마트 주입기 (다중 주입판)</h2>
            <p style="color:#64748b; margin-bottom:10px;">여러 개의 퀴즈나 지식을 한 번에 붙여넣어도 <b>자동으로 분해하여 개별 저장</b>합니다.</p>
        </div>
...
window.processUnifiedBulkAdd = async function() {
    ...
    try {
        ...
        // 🚨 [2] 다중 퀴즈 추출 엔진 ('문제 유형'을 기준으로 무한 분해)
        if (s2 || coreRemind) {
            let qBlocks = s2.split(/(?=문제\s*유형[:\s]*)/i).map(s => s.trim()).filter(Boolean);
            if(qBlocks.length === 0 && coreRemind) qBlocks.push(''); // 퀴즈 없이 리마인드만 있을 경우 대비
            ...
            for(let qIdx = 0; qIdx < qBlocks.length; qIdx++) {
                ...
                if(!qObj.text && !qObj.coreRemind) continue; // 빈 블록 건너뛰기
        ...
        // 🚨 [3] 다중 지식 재구성 추출 엔진 ('암기 코드'를 기준으로 무한 분해)
        if (s3) {
        ...
        // 🚨 [5] 다중 사례 분석: 4줄 단위로 묶어서 무한 분리 저장!
        if (s5) {
            ...
            if (dataRows.length > 0) {
                ...
                // 4줄(1세트) 단위로 잘라서 각각 새로운 사례 분석으로 저장합니다!
                for(let i=0; i < pureDataRows.length; i+=4) {
                    let sObj = { title:'사례 분석 ' + (Math.floor(i/4)+1), category:'미분류', sit_c:'', sit_n:'', pri_c:'', pri_n:'', sta_c:'', sta_n:'', pnt_c:'', pnt_n:'' };
                ...
                let errObj = { title:'사례 분석 오류', category:'미분류', sit_c:s5.trim(), sit_n:"⚠️ 표 데이터 인식 실패.", pri_c:'', pri_n:'', sta_c:'', sta_n:'', pnt_c:'', pnt_n:'' };
