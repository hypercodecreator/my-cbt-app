// [v8.9.0] app-ui.js: Dual D&D UX & Hierarchical Accordion
...
window.renderBaseUI = function() {
    ...
    vHead.innerHTML = `<span style="background:#10b981; padding:3px 10px; border-radius:6px; font-weight:900; font-size:0.85em; margin-right:10px;">v8.9.0</span> <span style="font-size:0.9em;">[Left/Right Dual D&D & Nested Folders UX]</span>`;
    ...
    // 🚨 왼쪽(순서스왑), 오른쪽(하위폴더 편입) 시각 효과 CSS
    if (!document.getElementById('core-styles')) {
        const s = document.createElement('style'); s.id = 'core-styles';
        s.innerHTML = `
            .rhythm-bar-fill { height:100%; background:linear-gradient(90deg, #ec4899, #8b5cf6); transition: width linear; }
            .drag-over-left { border-left: 8px solid #3b82f6 !important; background: linear-gradient(90deg, rgba(59,130,246,0.15) 0%, transparent 100%) !important; }
            .drag-over-right { border-right: 8px solid #10b981 !important; background: linear-gradient(270deg, rgba(16,185,129,0.15) 0%, transparent 100%) !important; }
            .drag-over-item { border-top: 4px solid #f59e0b !important; }
    ...
    sv.innerHTML = `
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
            <h2 style="margin:0; font-size:1.5em;">📚 과목 리스트</h2>
            <button class="button small-button primary-button" style="background:#8b5cf6; border:none;" onclick="window.toggleInlineCreateUI('subjects')">+ 새 폴더 생성</button>
        </div>
        <div id="inline-create-subjects" class="hidden" style="margin-bottom:20px; padding:20px; background:#f1f5f9; border-radius:12px; border:2px dashed #cbd5e1; display:flex; gap:10px;">
            <input type="text" id="create-inp-subjects" placeholder="새 폴더명 (슬래시/로 하위폴더 지정 가능)" style="flex:1; padding:12px; border-radius:8px; border:1px solid #cbd5e1;">
            <button class="button primary-button" onclick="window.execInlineCreate('subjects')">생성</button>
            <button class="button light-button" onclick="window.toggleInlineCreateUI('subjects')">취소</button>
        </div>
